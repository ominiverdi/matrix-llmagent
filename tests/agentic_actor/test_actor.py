...
from matrix_llmagent.providers import ModelRouter, ModelSpec
...
class TestAPIAgent:
    ...
    async def test_agent_tool_use_flow(self, agent, api_type):
        ...
        with patch.object(
            ...
            with patch(
                "matrix_llmagent.agentic_actor.actor.execute_tool", new_callable=AsyncMock
            ) as mock_tool:
    ...
    async def test_agent_api_error_handling(self, agent):
        ...
        with patch.object(
            ...
            with pytest.raises(RuntimeError, match="Agent took too many turns to research"):
